...
mix_estimate$variant_id = dat$geno_name
```
...
for(i in 1 : ncol(df)) {
  ...
  message('[mixqtl] --> check diff, sum(abs diff), of column ', colnames(df)[i], ': ' , sum(abs(df[, i] - mix_estimate[, i])))
}
```
...
df2 = inner_join(df, mix_estimate, by = 'variant_id', suffix = c('.old', '.mixqtl.wrap'))
plist = list()
for(i in c('trc', 'asc', 'meta')) {
  plist[[length(plist) + 1]] = df2 %>% ggplot() + geom_point(aes_string(x = paste0('bhat.', i, '.old'), y = paste0('bhat.', i, '.mixqtl.wrap')), alpha = .1) + th  + coord_equal()
}
for(i in c('trc', 'asc', 'meta')) {
  plist[[length(plist) + 1]] = df2 %>% ggplot() + geom_point(aes_string(x = paste0('pval.', i, '.old'), y = paste0('pval.', i, '.mixqtl.wrap')), alpha = .1) + scale_x_log10() + scale_y_log10() + th + coord_equal()
}
plot_grid(plotlist = plist)
```
